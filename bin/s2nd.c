...
void usage()
{
    ...
    fprintf(stderr, "  --no-session-cache\n");
    fprintf(stderr, "    Do not use session caching to resume\n");
    fprintf(stderr, "  --no-session-tickets\n");
...
int main(int argc, char *const *argv)
{
    ...
    struct option long_options[] = {
        ...
        {"no-session-cache", no_argument, 0, 'S'},
    ...
    while (1) {
        int option_index = 0;
        int c = getopt_long(argc, argv, "c:hmnst:d:i:ST", long_options, &option_index);
        if (c == -1) {
            break;
        }
        ...
        case 'S':
            conn_settings.no_session_caching = 1;
            break;
        case 'T':
    ...
    if (!conn_settings.no_session_caching) {
